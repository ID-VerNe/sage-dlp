"""
This module defines centralized constants used across the SageDLP application.
...
def get_asset_path(asset_relative_path: str) -> Path:
    ...
    try:
        ...
        try:
            ...
            package_path = resources.files('sage_dlp')
    ...
    # Go up from utils to sage_dlp root, then to asset
    sage_root = current_file.parent.parent.parent
    asset_path = sage_root / asset_relative_path
...
if OS_NAME == "Windows":
    ...
    APP_DIR: Path = Path(os.environ.get("LOCALAPPDATA", USER_HOME_DIR / "AppData" / "Local")) / "SageDLP"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "sage_dlp_config.json"
    APP_HISTORY_FILE: Path = APP_DATA_DIR / "sage_history.json"
    ...
    # Always use user data directory for app data, logs, config, and binaries
    # Even when frozen, we don't want to create these folders next to the executable
    APP_DIR: Path = USER_HOME_DIR / "Library" / "Application Support" / "SageDLP"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "sage_dlp_config.json"
    ...
    # Always use user data directory for app data, logs, config, and binaries
    # Even when frozen, we don't want to create these folders next to the executable
    APP_DIR: Path = USER_HOME_DIR / ".local" / "share" / "SageDLP"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "sage_dlp_config.json"
