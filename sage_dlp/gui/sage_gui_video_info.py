# -*- coding: utf-8 -*-
"""
Video information mixin for SageDLP GUI.
Contains the video info section UI elements.
"""
...
class VideoInfoMixin:
    def setup_video_info_section(self) -> QHBoxLayout:
        ...
        self.thumbnail_label.setStyleSheet("border: 2px solid #e2e8f0; border-radius: 8px;")
        ...
        self.subtitle_select_btn.setStyleSheet(
            ...
            QPushButton:hover { background-color: #eff4fa; }
        ...
        self.selected_subs_label.setStyleSheet("color: #0f172a; padding-left: 5px;")
        ...
        # Add small spacing between subtitle section and the end
    ...
    def update_video_info(self, info) -> None:
        ...
        if hasattr(self, "is_playlist") and self.is_playlist:
            ...
            channel_label_text = _("video_info.channel")
            unknown_channel_text = _("video_info.unknown_channel")
            self.channel_label.setText(f"{channel_label_text}: {info.get('uploader', unknown_channel_text)}")
            views_label_text = _("video_info.views")
            self.views_label.setText(f"{views_label_text}: {formatted_views}")
            likes_label_text = _("video_info.likes")
            self.like_count_label.setText(f"{likes_label_text}: {formatted_likes}")
            date_label_text = _("video_info.upload_date")
            self.date_label.setText(f"{date_label_text}: {formatted_date}")
            duration_label_text = _("video_info.duration")
            self.duration_label.setText(f"{duration_label_text}: {duration_str}")
