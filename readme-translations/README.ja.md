...
<a id="なぜsage-dlpなのか"></a>
## ❓ なぜ SageDLP なのか?

SageDLPは、**シンプルでありながら強力なYouTubeダウンローダー**を求めるユーザーのために設計されています。他のツールとは異なり、以下の機能を提供します：
...
> [👉 最新リリースをダウンロード](https://github.com/oop7/SageDLP/releases/latest)
...
2. **ポータブル版 (`.zip`)**: アーカイブを任意の場所に展開し、`sage-dlp.exe` を実行します。
...
- **DMG インストーラー (`.dmg`)**: ダブルクリックしてマウントし、`SageDLP.app` をアプリケーションフォルダにドラッグします。
- **App アーカイブ (`.zip`)**: ZIP を展開し、`SageDLP.app` をアプリケーションフォルダに移動します。
...
Dailymotion、CBC Gem、TikTok など、yt-dlp がサポートするサイトからの URL を SageDLP に受け入れさせたい場合は、ジェネリックモードを使用します。
...
- ジェネリックモードは、SageDLP 内部の URL バリデーションのみを変更します。対象サイトは、インストールされている yt-dlp バージョンでサポートされている必要があります。
...
- **ジェネリックモード:** SageDLP が yt-dlp でサポートされている YouTube 以外のサイトから分析およびダウンロードできるようにします。**Download Settings → Generic Mode** から有効にします。
...
SageDLP はグローバルに対応するため、**14 言語**をサポートしています。**Custom Options → Language** で好みの言語を選択してください。
...
- **ダウンロード後にビデオとオーディオファイルが分かれている:** これは FFmpeg が不足しているか、検出されない場合に発生します。SageDLP は、高品質のビデオとオーディオストリームを結合するために FFmpeg を必要とします。
  - **解決策:** FFmpeg がインストールされており、システムの PATH でアクセス可能であることを確認してください。Windows ユーザーにとって最も簡単なオプションは、FFmpeg が同梱されている `SageDLP-v<version>-ffmpeg.exe` ファイルをダウンロードすることです。
...
**安全な選択肢：**
- ✅ **pip インストールを使用する:** `pip install sage-dlp` (推奨)
...
2.  **次のコマンドを入力します** が、まだ Enter は押さないでください。最後にスペースを含めるようにしてください：
    ...
    ```
3.  **Finder ウィンドウから `SageDLP.app` ファイルをドラッグし**、ターミナルウィンドウに直接ドロップします。これにより、正しいファイルパスが自動的に貼り付けられます。
4.  **Enter を押して** コマンドを実行します。
5.  **SageDLP.app を再度開いてみてください。** 正しく起動するはずです。
...
SageDLP があなたの時間を節約できたなら、プロジェクトのスポンサーになることを検討してください。スポンサーシップは、開発時間、全プラットフォームでのテスト、および将来の改善に役立てられます。
...
- `sage-dlp/languages/<code>.json` を編集して、アプリの文字列を同期させる
...
このドキュメントでは、SageDLP の整理されたフォルダ構造について説明します。
...
└── 📁 sage-dlp/                    # ソースコードパッケージ
    ...
    ├── 📁 core/                  # コアビジネスロジック
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_deno.py     # Deno 統合
    │   ├── 📄 sage-dlp_downloader.py # ダウンロード機能
    │   ├── 📄 sage-dlp_ffmpeg.py   # FFmpeg 統合
    │   ├── 📄 sage-dlp_utils.py    # ユーティリティ関数
    │   └── 📄 sage-dlp_yt_dlp.py   # yt-dlp 統合
    ├── 📁 gui/                   # ユーザーインターフェースコンポーネント
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_gui_main.py # アプリメインウィンドウ
    │   └── 📁 sage-dlp_gui_dialogs/ # ダイアログクラス
    ├── 📁 utils/                 # ユーティリティモジュール
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_config_manager.py # 設定管理
    │   └── 📄 sage-dlp_logger.py   # ロギングツール
