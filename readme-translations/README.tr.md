...
<img src="../branding/screenshots/main.png" width="800" alt="SageDLP Arayüzü"/>
...
<a id="neden-sage-dlp"></a>
## ❓ Neden SageDLP?

SageDLP, **basit ama güçlü bir YouTube indiricisi** isteyen kullanıcılar için tasarlanmıştır. Diğer araçların aksine şunları sunar:
...
SageDLP'i PyPI üzerinden yükleyin:
...
> [👉 En Son Sürümü İndir](https://github.com/oop7/SageDLP/releases/latest)
...
2. **Taşınabilir Sürüm (`.zip`)**: Dosyayı istediğiniz yere çıkarın ve `sage-dlp.exe` dosyasını çalıştırın.
...
- **DMG Yükleyici (`.dmg`)**: Bağlamak için çift tıklayın ve `SageDLP.app` dosyasını Uygulamalar klasörünüze sürükleyin.
- **Uygulama Arşivi (`.zip`)**: ZIP dosyasını çıkarın ve `SageDLP.app` dosyasını Uygulamalar klasörünüze taşıyın.
...
SageDLP'in Dailymotion, TikTok ve diğerleri gibi yt-dlp tarafından desteklenen sitelerden gelen URL'leri kabul etmesini istediğinizde Genel Modu kullanın.
...
- Genel Mod sadece SageDLP içindeki URL doğrulamasını değiştirir. Hedef site hala yüklü yt-dlp sürümünüz tarafından desteklenmelidir.
...
- **Genel Mod:** SageDLP'in yt-dlp tarafından desteklenen YouTube dışındaki siteleri analiz etmesine ve indirmesine olanak tanır. **Download Settings → Generic Mode** bölümünden etkinleştirin.
...
SageDLP, küresel erişim için **14 dili** destekler. Tercih ettiğiniz dili **Custom Options → Language** bölümünden seçin.
...
- **İndirme sonrası video ve ses dosyaları ayrı:** Bu durum FFmpeg eksik olduğunda veya algılanmadığında olur. SageDLP, yüksek kaliteli video ve ses akışlarını birleştirmek için FFmpeg gerektirir.
  - **Çözüm:** FFmpeg'in kurulu olduğundan ve sistem PATH'inizde erişilebilir olduğundan emin olun. Windows kullanıcıları için en kolay seçenek, FFmpeg ile birlikte gelen `SageDLP-v<sürüm>-ffmpeg.exe` dosyasını indirmektir.
...
**Güvenli Seçenekler:**
- ✅ **pip kurulumunu kullanın:** `pip install sage-dlp` (önerilir)
...
2.  **Aşağıdaki komutu yazın**, ancak henüz Enter tuşuna **BASMAYIN**. Sonundaki boşluğu eklediğinizden emin olun:
    ...
    ```
3.  **`SageDLP.app` dosyasını Finder penceresinden sürükleyin** ve doğrudan Terminal penceresine bırakın. Bu, doğru dosya yolunu otomatik olarak yapıştıracaktır.
4.  Komutu çalıştırmak için **Enter tuşuna basın**.
5.  **SageDLP.app'i tekrar açmayı deneyin.** Artık düzgün bir şekilde çalışmalıdır.
...
SageDLP size zaman kazandırıyorsa, projeye sponsor olmayı düşünün. Sponsorluklar geliştirme süresini, tüm platformlarda test yapmayı ve gelecekteki iyileştirmeleri karşılamaya yardımcı olur.
...
- Uygulama dizelerini `sage-dlp/languages/<code>.json` dosyasını düzenleyerek senkronize tutun
- Diliniz eksikse, `README.md` dosyasından başlayın ve `README.<code>.md` dosyasını oluşturun

<details>
<summary>📂 Proje Yapısı</summary>

## SageDLP - Proje Yapısı

Bu belge SageDLP'in düzenli klasör yapısını detaylandırır.
...
└── 📁 sage-dlp/                    # Kaynak kod paketi
    ...
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_deno.py     # Deno entegrasyonu
    │   ├── 📄 sage-dlp_downloader.py # İndirme işlevselliği
    │   ├── 📄 sage-dlp_ffmpeg.py   # FFmpeg entegrasyonu
    │   ├── 📄 sage-dlp_utils.py    # Yardımcı fonksiyonlar
    │   └── 📄 sage-dlp_yt_dlp.py   # yt-dlp entegrasyonu
    ├── 📁 gui/                   # Kullanıcı arayüzü bileşenleri
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_gui_main.py # Ana uygulama penceresi
    │   └── 📁 sage-dlp_gui_dialogs/ # Diyalog sınıfları
    ├── 📁 utils/                 # Yardımcı modüller
    │   ├── 📄 __init__.py        
    │   ├── 📄 sage-dlp_config_manager.py # Yapılandırma yönetimi
    │   └── 📄 sage-dlp_logger.py   # Günlük tutma araçları
