...
class YTSageApp(QMainWindow, FormatTableMixin, VideoInfoMixin, AnalysisMixin):  # Inherit from mixins
    ...
    def init_ui(self) -> None:
        ...
        self.llm_segment_checkbox.setToolTip("Enable LLM-powered subtitle segmentation (requires LLM config)")
    ...
    def start_download(self) -> None:
        ...
        # LLM segmentation
        self.download_thread.llm_segment_enabled = self.llm_segment_checkbox.isChecked() if hasattr(self, "llm_segment_checkbox") else False
        if self.download_thread.llm_segment_enabled:
            llm_config = {
                "mode": ConfigManager.get("llm_mode") or "rule",
    ...
    def show_update_dialog(self, latest_version, release_url, changelog) -> None:  # Added changelog parameter
        ...
        # Title
        title_label = QLabel(f"<h2 style='color: #0984e3; margin: 0;'>{_('update_dialog.title')}</h2>")
        ...
        message_label = QLabel(
            ...
            f"<b style='color: #2d3436;'>{_('update_dialog.new_version_available')}</b><br><br>"
            f"<span style='color: #636e72;'>{_('update_dialog.current_version_label')} <b style='color: #2d3436;'>{self.version}</b></span><br>"
            f"<span style='color: #636e72;'>{_('update_dialog.latest_version_label')} <b style='color: #00b894;'>{latest_version}</b></span>"
            f"</div>"
        )
        ...
        # Changelog Section
        changelog_label = QLabel(f"<b style='color: #2d3436; font-size: 14px;'>{_('update_dialog.changelog')}:</b>")
        ...
        if self.audio_button.isChecked():
            ...
            if hasattr(self, "llm_segment_checkbox"):
                ...
                if not can_enable:
                    ...
                    self.llm_segment_checkbox.setToolTip("Select subtitles first")
                else:
                    self.llm_segment_checkbox.setToolTip("Enable LLM-powered subtitle segmentation")
    ...
    def _apply_blur_to_pixmap(self, source_pixmap: QPixmap, radius: int) -> QPixmap:
        """Deprecated: kept for compatibility. Use run_dialog_with_blur instead."""
